 ...
 * @author ShengHao
 *
 *         2016年12月15日-下午3:52:59
 */
public class HttpRequestDeviceUtils {

	/** Wap网关Via头信息中特有的描述信息 */
	private static String MOBILE_GATE_WAY_HEADERS[] = new String[] { "ZXWAP", // 中兴提供的wap网关的via信息，例如：Via=ZXWAP
																				// GateWayZTE
																				// Technologies，
			"http://chinamobile.com", // 中国移动的诺基亚wap网关，例如：Via=WTP/1.1
										// http://GDSZ-PB-GW003-WAP07.gd.chinamobile.com
										// (Nokia WAP Gateway 4.1
										// CD1/ECD13_D/4.1.04)
			"http://monternet.com", // 移动梦网的网关，例如：Via=WTP/1.1
									// BJBJ-PS-WAP1-GW08.bj1.monternet.com.
									// (Nokia WAP Gateway 4.1
									// CD1/ECD13_E/4.1.05)
	...
	private static String[] PC_HEADERS = new String[] { "Windows 98", "Windows ME", "Windows 2000", "Windows XP",
			"Windows NT", "Ubuntu" };

	/** 手机浏览器的User-Agent里的关键词 */
	private static String[] MOBILE_USER_AGENTS = new String[] { "Nokia", // 诺基亚，有山寨机也写这个的，总还算是手机，Mozilla/5.0
																			// (Nokia5800
																			// XpressMusic)UC
																			// AppleWebkit(like
																			// Gecko)
	...
	public static boolean isMobileDevice(HttpServletRequest request) {
		...
		for (int i = 0; StringUtils.isNotBlank(via) && i < MOBILE_GATE_WAY_HEADERS.length; i++) {
			if (via.contains(MOBILE_GATE_WAY_HEADERS[i])) {
				mobileFlag = true;
				break;
			}
		}
		for (int i = 0; !mobileFlag && StringUtils.isNotBlank(userAgent) && i < MOBILE_USER_AGENTS.length; i++) {
			if (userAgent.contains(MOBILE_USER_AGENTS[i])) {
				mobileFlag = true;
				break;
			}
		}
		for (int i = 0; StringUtils.isNotBlank(userAgent) && i < PC_HEADERS.length; i++) {
	...
	public static boolean isMobileRequest(HttpServletRequest request) {
		return request.getRequestURI().contains("payPortal/mobile");
	}
